...
def sendsms(command):
	...
	contacts = {'shoaib':9167075383,'salman':9664366965,'mobin':8652747053,'sahil':9834417011}
	
	for i in contacts:
		if i in command:
			command = command.replace("send text to","").replace('that',"").replace(i,"")
			break
	else:
...
def assistant(command):
    ...
    if command == "hello1" or command == "hi1" or command=="hey1":
    	...
    	talkToMe('Started. Press Ctrl-C to exit or just say exit')
    	startTime = time.time()
    	try:
    		while True:
    		...
    		talkToMe(str(totalTime))

    elif ('what' in command or 'where' in command or 'who' in command or 'whose' in command or 'when' in command or 'how' in command or 'whom' in command or 'why' in command or 'which' in command):
        ...
        fw = open(r'D:\\ArmanK\\python_programs\\SUBLIME_FILES\\SpeechRecog\\.facts.txt',encoding="utf8")
        facts = fw.read()
        facts = facts.split('\n')
        while True:    
        ...
        fw = open(r'D:\\ArmanK\\python_programs\\SUBLIME_FILES\\SpeechRecog\\.quotes.txt','r')
        vocab = fw.read()
        vocab = vocab.split('\n')
        ...
        fw = open(r'D:\\ArmanK\\python_programs\\SUBLIME_FILES\\SpeechRecog\\.vocab.txt','r')
        vocab = fw.read()
        vocab = vocab.split('\n')
        ...
        f = open(r'D:\\ArmanK\\python_programs\\SUBLIME_FILES\\SpeechRecog\\Notes.txt','a')
        t = time.asctime(time.localtime(time.time()))
        ...
        f = open(r'D:\\ArmanK\\python_programs\\SUBLIME_FILES\\SpeechRecog\\Notes.txt','r')
        talkToMe(f.read())
        f.close()

    elif 'clear my notes' in command:
        f = open(r'D:\\ArmanK\\python_programs\\SUBLIME_FILES\\SpeechRecog\\Notes.txt','w')
        f.close()
        talkToMe("Done")
    ...
    elif 'brightness' in command or 'light' in command:
    	if 'low' in command or 'dim' in command or 'decrease' in command or 'less' in command or 'minimun' in command or 'min' in command:
    		os.system('nircmd.exe setbrightness 5')
    	if 'medium' in command or 'increase' in command:
    		os.system('nircmd.exe setbrightness 50')
    	if 'full' in command or 'max' in command or 'maximum' in command or 'max' in command:
